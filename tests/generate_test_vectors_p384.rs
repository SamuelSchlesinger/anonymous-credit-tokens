...
//!
//!   cargo test --test generate_test_vectors_p384
//!
//! Use `-- --nocapture` to also see the output on stdout.

use anonymous_credit_tokens::p384::*;
...
fn generate_test_vectors() {
    ...
    const START: &str = "<!-- P384_TEST_VECTORS_START -->";
    const END: &str = "<!-- P384_TEST_VECTORS_END -->";

    let start_idx = spec
        .find(START)
        .expect("missing P384_TEST_VECTORS_START marker in spec");
    let end_idx = spec
        .find(END)
        .expect("missing P384_TEST_VECTORS_END marker in spec");
    ...
    if existing == expected {
        ...
        panic!(
            "Test vectors in the spec are out of date!\n\n\
             ...
             \x20   UPDATE_TEST_VECTORS=1 cargo test --test generate_test_vectors_p384\n\n\
