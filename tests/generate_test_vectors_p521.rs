...
//!
//!   cargo test --test generate_test_vectors_p521
//!
//! Use `-- --nocapture` to also see the output on stdout.

use anonymous_credit_tokens::p521::*;
...
fn generate_test_vectors() {
    ...
    const START: &str = "<!-- P521_TEST_VECTORS_START -->";
    const END: &str = "<!-- P521_TEST_VECTORS_END -->";

    let start_idx = spec
        .find(START)
        .expect("missing P521_TEST_VECTORS_START marker in spec");
    let end_idx = spec
        .find(END)
        .expect("missing P521_TEST_VECTORS_END marker in spec");
    ...
    if existing == expected {
        ...
        panic!(
            "Test vectors in the spec are out of date!\n\n\
             ...
             \x20   UPDATE_TEST_VECTORS=1 cargo test --test generate_test_vectors_p521\n\n\
