...
use k256_crate::{ProjectivePoint, U256};
use proptest::prelude::*;
use rand_core::OsRng;

impl TestCiphersuite for Secp256k1 {
...
// Instantiate all common tests for Secp256k1
crate::instantiate_tests!(Secp256k1);

// ── Local wrappers for proptest helpers ─────────────────────────────────

fn scalar_from_u128(v: u128) -> Scalar {
    Secp256k1::scalar_from_u128(v)
}

fn bits_of<const L: usize>(s: Scalar) -> [subtle::Choice; L] {
    Secp256k1::bits_of::<L>(s)
}

fn test_context() -> Scalar {
    <Secp256k1 as TestCiphersuite>::test_context()
}
