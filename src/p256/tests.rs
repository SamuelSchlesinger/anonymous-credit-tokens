...
// Instantiate all common tests for P256
crate::instantiate_tests!(P256);

// ── Local wrappers for proptest helpers ─────────────────────────────────

/// Wrapper for `P256::scalar_from_u128` for backward compatibility in tests.
fn scalar_from_u128(v: u128) -> Scalar {
    P256::scalar_from_u128(v)
}

/// Wrapper for `P256::bits_of` for backward compatibility in tests.
fn bits_of<const L: usize>(s: Scalar) -> [subtle::Choice; L] {
    P256::bits_of::<L>(s)
}

/// Wrapper that calls the `TestCiphersuite` implementation.
fn test_context() -> Scalar {
    <P256 as TestCiphersuite>::test_context()
}
...
proptest! {
    ...
    fn prop_transcript_determinism(
        ...
        let challenge1 = Transcript::<P256>::with(&params.transcript_base, &label, |transcript| {
            for point in &points {
                transcript.add_element(point);
            }
        });

        let challenge2 = Transcript::<P256>::with(&params.transcript_base, &label, |transcript| {
            for point in &points {
                transcript.add_element(point);
            }
        });
