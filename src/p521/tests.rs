...
// Instantiate all common tests for P521
crate::instantiate_tests!(P521);

// ── Local wrappers for proptest helpers ─────────────────────────────────

fn scalar_from_u128(v: u128) -> Scalar {
    P521::scalar_from_u128(v)
}
...
fn test_context() -> Scalar {
    <P521 as TestCiphersuite>::test_context()
}
...
fn scalar_strategy() -> impl Strategy<Value = Scalar> {
    // Generate 72 random bytes and reduce into the P-521 scalar field
    (prop::array::uniform32(any::<u8>()), prop::array::uniform32(any::<u8>()), prop::collection::vec(any::<u8>(), 8..=8))
        .prop_map(|(a, b, c)| {
            let mut wide = [0u8; 72];
            wide[..32].copy_from_slice(&a);
            wide[32..64].copy_from_slice(&b);
            wide[64..].copy_from_slice(&c);
            <Scalar as Reduce<U576>>::reduce(U576::from_be_slice(&wide))
        })
}
