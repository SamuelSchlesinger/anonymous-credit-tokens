...
//! Deterministic test vector generation for the CFRG spec (BLS12-381 appendix).
//!
//! Running this test regenerates the BLS12-381 test vectors and splices them
//! into the spec markdown automatically:
//!
//!   cargo test -p anonymous-credit-tokens-public --test generate_test_vectors
//!
//! Use `-- --nocapture` to also see the output on stdout.

use anonymous_credit_tokens_public::*;
...
fn generate_test_vectors() {
    ...
    let domain_separator = "ACT-public-v1:test:vectors:v0:2025-01-01";
    ...
    let response = private_key
        .issue::<8>(&params, &request, Scalar::from(c as u64), ctx, &mut rng)
        .unwrap();
    ...
    let (spend_proof, prerefund) = token.prove_spend::<8>(&params, Scalar::from(s as u64), &mut rng).unwrap();
    let spend_proof_cbor = spend_proof.to_cbor().unwrap();
    let prerefund_cbor = prerefund.to_cbor().unwrap();

    // Also verify the spend proof publicly before proceeding
    verify_spend_proof(&params, private_key.public(), &spend_proof).unwrap();

    let t: u128 = 10;
    let refund = private_key.refund::<8>(&params, &spend_proof, Scalar::from(t as u64), &mut rng).unwrap();
    ...
    const START: &str = "<!-- BLS_TEST_VECTORS_START -->";
    const END: &str = "<!-- BLS_TEST_VECTORS_END -->";
    ...
    match (start_idx, end_idx) {
        (Some(start), Some(end)) => {
            let existing = &spec[start + START.len()..end];
            let expected = format!("\n{md}");

            if existing == expected {
                println!("BLS12-381 test vectors in spec are up to date.");
            } else if std::env::var("UPDATE_TEST_VECTORS").as_deref() == Ok("1") {
                let mut new_spec = String::with_capacity(spec.len() + md.len());
                new_spec.push_str(&spec[..start + START.len()]);
                new_spec.push_str(&expected);
                new_spec.push_str(&spec[end..]);

                std::fs::write(spec_path, &new_spec).expect("could not write spec markdown");

                println!("{md}");
                println!("--- wrote BLS12-381 test vectors to {spec_path}");
            } else {
                panic!(
                    "BLS12-381 test vectors in the spec are out of date!\n\n\
                     The generated test vectors do not match the content between\n\
                     {START} and {END}\n\
                     in {spec_path}.\n\n\
                     To update the spec with the new vectors, re-run with:\n\n\
                     \x20   UPDATE_TEST_VECTORS=1 cargo test -p anonymous-credit-tokens-public --test generate_test_vectors\n\n\
                     Then review and commit the changes."
                );
            }
        }
        _ => {
            // No markers yet — just print the test vectors and note they need to be added
            println!("BLS12-381 test vector markers not found in spec. Generated vectors:\n");
            println!("{md}");
            println!("Add <!-- BLS_TEST_VECTORS_START --> and <!-- BLS_TEST_VECTORS_END --> markers");
            println!("to the spec to enable automatic splicing.");
        }
    }
}
