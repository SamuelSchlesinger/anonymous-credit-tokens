...
$(A^*, e^*) = \left(\left(g \cdot K' \cdot h_4^{\mathit{ctx}}\right)^{1/(e^*+x)}, e^*\right)$
to the client.
Also prove that it computed this value correctly by proving that $\log_{A^*} \left(g \cdot K' \cdot h_4^{\mathit{ctx}}\right) = \log_g \left(g^{e^*} \cdot w\right)$ using a standard ``equality-of-discrete-logarithms'' proof.
That is, let $X_{A^*} = g \cdot K' \cdot h_4^{\mathit{ctx}}$ and $X_g = g^{e^*} \cdot w$. The issuer does:
\begin{enumerate}
    \item Choose $\alpha \leftarrow \Z_q$ and compute $Y_A:= (A^*)^{\alpha}$ and $Y_g:=g^\alpha$.
    \item Compute $\gamma:=H(e^*\|\mathit{ctx}\|A^*\|X_{A^*}\|X_g\|Y_A\|Y_g)$.
    \item Compute $z:=\gamma\cdot (x+e^*)+\alpha$, and send $\gamma, z$ to the client.
\end{enumerate}

\item ({\bf Client computes refund.}) The client verifies the proof $\gamma, z$ by computing $Y'_A:= (A^*)^z \cdot X_{A^*}^{-\gamma}$ and $Y'_g:=g^z \cdot X_g^{-\gamma}$ and then
checking if
$H( e^*\|\mathit{ctx}\|A^*\|X_{A^*}\|X_g\|Y'_A \| Y'_g ) \isequal \gamma$. If so, the client outputs $(A^*, e^*, k^*, r^*, c-s, \mathit{ctx})$; otherwise, the client outputs~$\perp$.
\end{itemize}
\end{document}
