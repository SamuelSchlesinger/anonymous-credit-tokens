...
\begin{itemize}
 ...
 %   \item $\vrfycred$, run by a client, takes as input a public key~$\pk$, a time period~$T$, and a credential~$\cred$; it outputs a bit.
    ...
    The issuer has as input its private key~$\sk$, a (non-negative integer) value~$s$, and a partial credit return $t$ with $0 \leq t \leq s$. The client has as input the issuer's public key~$\pk$, the same value~$s$, and a token $(\tok, c)$ with $s \leq c$.
    At the end of the protocol, the issuer outputs a nullifier~$k$ with $k=\perp$ indicating that it detected cheating by the client; the client outputs $(\tok', c')$ with $c' = c - s + t$ and $\tok=\perp$ indicating that it detected cheating by the issuer.
\end{itemize}
\end{definition}

Correctness requires that (in honest executions) issuance always succeeds in producing a token worth the appropriate number of credits, and that spending $s$ credits from token worth $c$ credits should succeed (at the issuer) and result in a new token worth $c'=c-s+t$ credits for the client, where $0 \leq t \leq s$ is a partial credit return chosen by the issuer.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        Run $\spend$ honestly between the issuer and a client on shared input~$s$ and with the client using token $(\tok_j, c_j)$. The issuer also chooses $t$ with $0 \leq t \leq s$. Let $k$ be the output of the issuer, and let $(\tok_i, c_i)$ be the output of the client. Set $\flag:=1$ if $k=\perp$ or $\tok_i=\perp$ or $c_i \neq c_j-s+t$ or $k$ was every previously output be the issuer following a $\spend$ query.
    \end{itemize}
\end{enumerate}
\end{definition}
...
\begin{enumerate}
    ...
    \item The client obtains a fresh BBS signature from the issuer on $(c-s+t, k^*)$ for a fresh~$k^*$, where $0 \leq t \leq s$ is a partial credit return chosen by the issuer. (Note this must be done even when $c-s+t=0$ if the client wants to maintain anonymity.)
\end{enumerate}
...
(random) generators $h_1, h_2, h_3, h_4 \in \G$. (These could be chosen by the issuer, or fixed public values.)
...
During issuance, a request context scalar $\mathit{ctx} \in \Z_q$ is provided and bound into the token via $h_4$.
...
$(A, e) = \left(\left(g \cdot h_1^c \cdot h_4^{\mathit{ctx}} \cdot K\right)^{1/(e+x)}, e\right)$
to the client.
It also proves that it computed this value correctly by proving that $\log_A \left(g \cdot h_1^c \cdot h_4^{\mathit{ctx}} \cdot K\right) = \log_g \left(g^e \cdot w\right)$ using a standard ``equality-of-discrete-logarithms'' proof.
That is, let $X_A = g \cdot h_1^c \cdot h_4^{\mathit{ctx}} \cdot K$ and $X_g = g^e \cdot w$. The issuer does:
\begin{enumerate}
    \item Choose $\alpha \leftarrow \Z_q$ and compute $Y_A:= A^{\alpha}$ and $Y_g:=g^\alpha$.
    \item Compute $\gamma:=H(c\|\mathit{ctx}\|e\|A\|X_A\|X_g\|Y_A\|Y_g)$.
...
checking if
$H( c\|\mathit{ctx}\|e\|A\|X_A\|X_g\|Y'_A \| Y'_g ) \isequal \gamma$. If so, the client outputs $(A, e, k, r, c, \mathit{ctx})$; otherwise, the client outputs~$\perp$.
\end{enumerate}

\medskip\noindent{\bf Spending.}
To spend $s<2^\ell$ credits using a token $(A, e, k, r, c, \mathit{ctx})$ for $c$ credits (with $s \leq c< 2^\ell$), the client and issuer do the following ($\send_1, \send_2$, and $\hash$ are initially empty):
\begin{itemize}
    ...
    \begin{enumerate}
        \item The client chooses $r_1, r_2, c', r', e', r'_2, r'_3 \leftarrow \Z_q$. It then sets $B:= g \cdot h_1^c h_2^k h_3^r h_4^{\mathit{ctx}}$, $A':=A^{r_1 r_2}$, $\bar B:=B^{r_1}$, $r_3 := r_1^{-1}$,
 $A_1:=(A')^{e'} \cdot \bar B^{r'_2}$, $A_2:= \bar B^{r'_3} \cdot h_1^{c'} h_3^{r'}$, $\send_1:=\send_1\|k\|\mathit{ctx}\|A'\|\bar B$, and $\hash:=\hash\|A_1\|A_2$.
...
The client stores $c-s, k^*, r^*, \mathit{ctx}$ to be used later below.


    \end{enumerate}
            \item ({\bf Verification by the issuer.}) The issuer, with secret key~$x$, parses $\send_1, \send_2$ as
            \begin{eqnarray*} & \send_1 = k\|\mathit{ctx}\|A'\|\bar B\|\com_0\|\cdots\|\com_{\ell-1} & \\
...
\begin{enumerate}
    \item Set $\bar A:=(A')^x$, $H_1:=g \cdot h_2^k \cdot h_4^{\mathit{ctx}}$, $A_1:= (A')^{\bar e} \bar B^{\bar r_2} \bar A^{-\gamma}$, $A_2:= \bar B^{\bar r_3} h_1^{\bar c} h_3^{\bar r} H_1^{-\gamma}$, and $\hash:=\hash\|A_1\|A_2$. 
    \item Do:
           \begin{enumerate}
...
\item ({\bf Issuing a refund.}) Let $K'$ be the value computed in the previous step, and recall that $K'=h_1^{c-s} h_2^{k^*} h_3^{r^*}$. The issuer chooses $t$ with $0 \leq t \leq s$ and $t < 2^\ell$ (a partial credit return). It can then use $K'$ and $t$ to issue a new token.
...
$(A^*, e^*) = \left(\left(g \cdot K' \cdot h_1^t \cdot h_4^{\mathit{ctx}}\right)^{1/(e^*+x)}, e^*\right)$
to the client.
Also prove that it computed this value correctly by proving that $\log_{A^*} \left(g \cdot K' \cdot h_1^t \cdot h_4^{\mathit{ctx}}\right) = \log_g \left(g^{e^*} \cdot w\right)$ using a standard ``equality-of-discrete-logarithms'' proof.
That is, let $X_{A^*} = g \cdot K' \cdot h_1^t \cdot h_4^{\mathit{ctx}}$ and $X_g = g^{e^*} \cdot w$. The issuer does:
\begin{enumerate}
    \item Choose $\alpha \leftarrow \Z_q$ and compute $Y_A:= (A^*)^{\alpha}$ and $Y_g:=g^\alpha$.
    \item Compute $\gamma:=H(e^*\|t\|\mathit{ctx}\|A^*\|X_{A^*}\|X_g\|Y_A\|Y_g)$.
    \item Compute $z:=\gamma\cdot (x+e^*)+\alpha$, and send $\gamma, z, t$ to the client.
\end{enumerate}

\item ({\bf Client computes refund.}) The client reconstructs $X_{A^*} = g \cdot K' \cdot h_1^t \cdot h_4^{\mathit{ctx}}$ using $t$ received from the issuer, and verifies the proof $\gamma, z$ by computing $Y'_A:= (A^*)^z \cdot X_{A^*}^{-\gamma}$ and $Y'_g:=g^z \cdot X_g^{-\gamma}$ and then
checking if
$H( e^*\|t\|\mathit{ctx}\|A^*\|X_{A^*}\|X_g\|Y'_A \| Y'_g ) \isequal \gamma$. If so, the client outputs $(A^*, e^*, k^*, r^*, c-s+t, \mathit{ctx})$; otherwise, the client outputs~$\perp$.
\end{itemize}
\end{document}
