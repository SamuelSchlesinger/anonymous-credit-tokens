...
fn issuance_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("issuance", |b| {
        b.iter(|| {
            black_box(private_key.issue(&issuance_request, black_box(credit_amount), OsRng).unwrap());
        })
    });
}
...
fn token_creation_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("token_creation", |b| {
        b.iter(|| {
            black_box(preissuance.to_credit_token(private_key.public(), &issuance_request, &issuance_response).unwrap());
        })
    });
}
