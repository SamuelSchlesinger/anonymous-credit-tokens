// Shared full-benchmark template. Each benchmark binary does:
//   use anonymous_credit_tokens::<suite>::{Params, PreIssuance, PrivateKey, Scalar};
//   include!("bench_full.rs");
...
fn create_params() -> Arc<Params> {
    Arc::new(
        Params::new("bench-org", "bench-service", "bench-env", "2024-01-01").unwrap(),
    )
}
    ...
    c.bench_function("token_creation", |b| {
        b.iter_batched(
            || {
                ...
                let credit_amount = Scalar::from(thread_rng().gen_range(10u128..1000));
...
// ---------------------------------------------------------------------------
// L-dependent benchmarks — one BenchmarkGroup per operation, parameterised
// by L so criterion generates scaling charts automatically.
// ---------------------------------------------------------------------------
    ...
    ($group:expr, $params:expr, $l:literal) => {
        $group.bench_with_input(BenchmarkId::from_parameter($l), &$l, |b, _| {
            b.iter_batched(
                ...
                |(pk, req, credit)| {
                    black_box(
                        pk.issue::<$l>(&$params, &req, credit, Scalar::ZERO, OsRng)
                            .unwrap(),
                    )
                },
                BatchSize::SmallInput,
